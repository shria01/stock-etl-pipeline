...
const tocItems = [
  ...
  ['performance', '04', 'Fixed holdout'],
  ['calibration', '05', 'Calibration'],
  ['coefficients', '06', 'Coefficients'],
  ['audit', '07', 'Evaluation'],
  ['experiments', '08', 'Experiments'],
  ['survival', '09', 'Survival-v1'],
  ['scope', '10', 'Scope'],
];
...
const featureGroups = [
  ['Event severity', 'drop_pct, event_max_drawdown_pct', 'Measures how deep the selloff was'],
  ['Drawdown speed', 'drawdown_velocity_pct_per_day', 'Distinguishes sharp drops from slow declines'],
  ['Momentum', 'prior_90d_return, relative_prior_90d_return', 'Captures the pre-event trend'],
  ['Volatility', 'volatility_90d', 'Measures instability before prediction'],
  ['Relative performance', 'relative_drop_pct, sector_relative_drop_pct', 'Separates stock weakness from market and sector moves'],
  ['Technical position', 'distance_from_52w_high', 'Captures distance from prior highs'],
  ['Sector context', 'one-hot sector indicators', 'Allows recovery patterns to vary by sector'],
];
  ...
  return (
    ...
    <article className="min-w-0 px-8 py-12 text-[#0B1220] sm:px-12">
      ...
      <section className="section-space-lg border-t border-[#D9E2EA]">
        ...
        <p className="mt-5 text-[15px] leading-7 text-[#52637A]">The final holdout includes 361 events from Q3 2023 through Q4 2025, with prediction dates ranging from September 29, 2023 to December 31, 2025. Every event had a fully observed 180-day outcome window. The analysis covers stocks currently in the S&P 500 and is intended for historical recovery analysis and ranking, not automated trading.</p>
      </section>

      <Section eyebrow="01 · Prediction task" title="A completed-quarter decision point"><p className={bodyCopy}>A drawdown event is created when a current S&amp;P 500 ticker falls 15% or more during a calendar quarter. The model waits until the quarter is complete, then predicts whether the stock will recover to its quarter-start baseline during the next 180 days.</p><Timeline /><p className={`mt-6 ${bodyCopy}`}>Events already recovered before the prediction date are excluded. Events without a full 180-day follow-up window are also excluded from the classifier dataset.</p></Section>
